...
export const feelsTemp = (temp: number, feelTemp: number): string => {
  if (feelTemp > temp) {
    return 'warmer';
  } else if (feelTemp < temp) {
    return 'colder';
  } else {
    return 'the same';
  }
...
export const getPressure = (pressure: number,standard: number=1013 ): string => {
  if (pressure > standard) {
    return 'Higher than';
  } else if (pressure > standard) {
    return 'Take care of "Soroche". Pressure is Less than';
  } else {
    return 'Equal to';
  }
};
